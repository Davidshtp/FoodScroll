...
export interface GetAvailableOrdersInput {
  userId: string;
  role: string;
}

export interface GetAvailableOrdersOutput {
...
export class GetAvailableOrdersUseCase {
  ...
  async execute(input: GetAvailableOrdersInput): Promise<GetAvailableOrdersOutput> {
    if (input.role !== 'DELIVERY') {
      throw new ForbiddenRoleError(input.role, 'access available orders');
    }

    const orders = await this.orderRepo.findAvailableForDelivery();
