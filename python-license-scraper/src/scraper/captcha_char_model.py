...
def default_char_model_dir() -> Path:
    """Return the absolute directory containing the trained captcha model."""
    repo_root = Path(__file__).resolve().parents[2]
    return repo_root / 'models' / 'captcha_char'
...
class CaptchaCharPredictor:
    ...
    def load(cls, model_dir: Path) -> Optional['CaptchaCharPredictor']:
        ...
        if not meta_path.exists() or not params_path.exists():
            logger.warning('CaptchaCharPredictor: model files not found at %s', model_dir)
            return None
        ...
        try:
            ...
            model.eval()
            logger.info('CaptchaCharPredictor loaded: %d classes, len=%d, best_acc=%s',
                        num_classes, captcha_len, meta.get('best_val_char_acc', 'N/A'))
            return cls(model=model, charset=charset, captcha_len=captcha_len)
        except Exception as e:
            logger.warning('CaptchaCharPredictor: failed to load model: %s', e)
            return None
    ...
    def predict(self, image_bytes: bytes) -> Tuple[str, float]:
        try:
            ...
            logger.warning('CaptchaCharPredictor predict failed: %s', e)
            return '', 0.0
