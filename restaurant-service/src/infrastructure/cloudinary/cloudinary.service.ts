...
export class CloudinaryService {
  ...
  async uploadLogo(file: Express.Multer.File): Promise<string> {
    return this.uploadImage(file, this.configService.get<string>(CLOUDINARY_LOGO_FOLDER) || 'avatars/restaurant-logos', {
      width: 400,
      height: 400,
      crop: 'fill',
      gravity: 'face',
  ...
  async uploadBanner(file: Express.Multer.File): Promise<string> {
    return this.uploadImage(file, this.configService.get<string>(CLOUDINARY_BANNER_FOLDER) || 'banners/restaurants', {
      width: 1200,
      height: 400,
      crop: 'fill',
    });
